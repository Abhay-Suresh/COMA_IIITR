...
* **Reference value ($V_{known}$):**
    * For `n=50`, use exact DP and B&B to compute the exact optimal value per instance. Use this as $V_{known}$.
    * For `n=200` and `n=500`, where exact computation may be infeasible, take the **best-known value** defined as the maximum value found across all algorithms (DP, B&B) and all runs for that specific instance.
* **Per-run gap:** For a given run returning value $V_{run}$, compute the relative optimality gap as:
    $gap = (V_{known} - V_{run}) / V_{known}$
    * If $V_{known} == 0$, set $gap = 0.0$ to avoid division by zero.
* **Per-instance aggregation:** If an algorithm is randomized, collapse its 10 runs on the same instance into summary stats (best, median, mean, std of $V_{run}$ and $gap$).
* **Per-algorithm summary:** For each algorithm, compute aggregated statistics of $gap$ across all instances of a given (n, distribution) pair: median $gap$, IQR, mean, and std.
...
* Test correctness on small instances by comparing DP and B&B results.
...
* **Algorithm runs:** For randomized algorithms (GA, ACO, SA), perform **10 independent runs per instance** and record the RNG seed for each run.
* **Metrics to record:**
...
* **Reporting & aggregation:** For each algorithm and (size, distribution) pair, aggregate per-instance gaps and runtimes across all runs.
    * Report median $\pm$ IQR and mean $\pm$ std.
    * Provide convergence plots (for metaheuristics) and boxplots per distribution.